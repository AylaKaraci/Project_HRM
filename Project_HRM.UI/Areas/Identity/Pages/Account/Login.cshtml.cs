...
namespace Project_HRM.UI.Areas.Identity.Pages.Account
{
    [AllowAnonymous]
    public class LoginModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    var user = _uow.employeeRepository.GetFirstOrDefault(u => u.Email == Input.Email.ToLower());

                    var userInfo = new SessionContext()
                    {
                        Email = user.Email,
                        FirstName = user.FirstName,
                        //TODO: Admin bilgisini dinamik olarak getir.
                        IsAdmin = false,
